...
cat("Dimensions de la base individus:", dim(base_individus), "\n")
cat("Dimensions de la base principale:", dim(base_principale), "\n")

# Vérification des valeurs manquantes par colonne
cat("\nNombre de valeurs manquantes par colonne (top 10) - base individus:\n")
na_count_ind <- sapply(base_individus, function(y) sum(length(which(is.na(y)))))
head(sort(na_count_ind, decreasing = TRUE), 10)

cat("\nNombre de valeurs manquantes par colonne (top 10) - base principale:\n")
na_count_prin <- sapply(base_principale, function(y) sum(length(which(is.na(y)))))
...
head(data.frame(original = names(base_individus), nettoyé = names(base_individus_clean)), 5)

cat("\nExemple de noms de colonnes avant/après nettoyage - Base Prin:\n")
head(data.frame(original = names(base_principale), nettoyé = names(base_principale_clean)), 5)
```

### 2.3 Suppression des colonnes et lignes inutiles
...
cat("Dimensions de la base Ind avant:", dim(base_individus), "\n")
cat("Dimensions de la base Ind après:", dim(base_individus_clean), "\n\n")
cat("Dimensions de la base Prin avant:", dim(base_principale), "\n")
cat("Dimensions de la base Prin après:", dim(base_principale_clean), "\n")

# Suppression des lignes presque vides (seuil de 95%)
base_individus_clean <- base_individus_clean %>%
  filter(rowSums(is.na(.)) < 0.95 * ncol(.))

Prin_clean <- base_principale_clean %>%
  filter(rowSums(is.na(.)) < 0.95 * ncol(.))

# Comparaison après la suppression des lignes
cat("\nDimensions de la base Ind après suppression des lignes:", dim(base_individus_clean), "\n")
cat("Dimensions de la base Prin après suppression des lignes:", dim(base_principale_clean), "\n")
...
lapply(base_individus_clean %>% select(starts_with("HH") & where(is.integer)) %>% head(5), class)
...
if (any(class(base_individus$HH_02) %in% c("haven_labelled", "labelled"))) {
  # Afficher les labels si la variable est labellisée
  cat("Labels pour la variable sexe:\n")
  print(val_labels(base_individus$HH_02))
}

# Distribution des sexes (fréquence)
sexe_table <- table(base_individus$HH_02, useNA = "always")
sexe_table
prop.table(sexe_table) * 100

# Distribution des statuts matrimoniaux
if (any(class(base_individus$HH_08) %in% c("haven_labelled", "labelled"))) {
  cat("Labels pour la variable état matrimonial:\n")
  print(val_labels(base_individus$HH_08))
}

etatmatri_table <- table(base_individus$HH_08, useNA = "always")
etatmatri_table
...
if (any(class(base_individus$HH_Educ02a) %in% c("haven_labelled", "labelled"))) {
  cat("Labels pour la variable fréquentation scolaire actuelle:\n")
  print(val_labels(base_individus$HH_Educ02a))
}

educ_actuelle_table <- table(base_individus$HH_Educ02a, useNA = "always")
educ_actuelle_table
prop.table(educ_actuelle_table) * 100

# Niveau d'éducation actuel
if (any(class(base_individus$HH_Educ03) %in% c("haven_labelled", "labelled"))) {
  cat("Labels pour la variable niveau d'éducation:\n")
  print(val_labels(base_individus$HH_Educ03))
}

education_table <- table(base_individus$HH_Educ03, useNA = "always")
education_table
prop.table(education_table) * 100

# Type d'école
if (any(class(base_individus$HH_Educ04a) %in% c("haven_labelled", "labelled"))) {
  cat("Labels pour la variable type d'école:\n")
  print(val_labels(base_individus$HH_Educ04a))
}

type_ecole_table <- table(base_individus$HH_Educ04a, useNA = "always")
type_ecole_table
...
sapply(base_individus[vars_num], function(x) sum(is.na(x)))
...
## 1. Analyse descriptive des variables qui composent le rCSI
...
## 3. Calcul de l'indice réduit des stratégies de survie (rCSI)
