...
Voici les packages installés et chargés pour ce projet : *readr*, *readxl*,*haven* et *dplyr* (Data Plyr ou dplyr fait référence à un package plus ancien *plyr* pour la manipulation de données). Le code ci-dessous s'assure que ces packages sont disponibles avant d'être utilisés dans le traitement des données :


```{r}
packages <- c("readr", "readxl", "dplyr","haven")
...
Le code ci-dessous permet de sauvegarder les données fusionnées sous formats .csv et .dta.
...
#Sauvegarde en Stata/ Il n'accepte pas certains caractères sur les variables donc...
# Vérifier les noms exacts des colonnes avant de les renommer
print(colnames(final_data), quote = TRUE)  # Affiche les noms avec guillemets

#️ Renommer les colonnes problématiques pour éviter les caractères interdits
final_data <- final_data %>%
  rename(
    OBJECTID = `OBJECTID *`,  # Remplace "OBJECTID *" par "OBJECTID"
    Shape = `Shape *`         # Remplace "Shape *" par "Shape"
  )

#colnames(final_data) #Checking

#Sauvegarder au format Stata (.dta)
write_dta(final_data, "TP5_G3_FinalMergedData.dta")
```
